...
capture = cv2.VideoCapture('mov1.mp4')
...
while True:
   ...
   lower_yellow = (15, 130, 120)  # 자료형은 튜플형태로(H,S,V)
   ...
   # img_bgr = cv2.imread('images/lane.png', cv2.IMREAD_COLOR)
   img_mask = np.zeros_like(img_dst)
   ...
   img_tmp = cv2.fillPoly(img_mask, pts, mask_color)
   img_org = cv2.bitwise_and(img_dst, img_tmp)

   img_our = cv2.cvtColor(img_org, cv2.COLOR_BGR2GRAY)
   img_pls = cv2.GaussianBlur(img_our, (5, 5), 0)
   # 이진화 수행
   _, img_binary = cv2.threshold(img_pls, 0, 255, cv2.THRESH_OTSU + cv2.THRESH_BINARY)
